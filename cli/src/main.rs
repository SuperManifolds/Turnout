...
fn main() -> Result<()> {
    ...
    if data.len() >= 32 && &data[0..4] == b"NRC1" {
        ...
        let uncompressed = u64::from_le_bytes(data[8..16].try_into().expect("8-byte slice"));
        let compressed = u64::from_le_bytes(data[16..24].try_into().expect("8-byte slice"));
        let checksum = u64::from_le_bytes(data[24..32].try_into().expect("8-byte slice"));
        println!("=== NRC1 Container ===");
        println!("  Model version:     {version}");
        println!("  Uncompressed size: {uncompressed} bytes");
